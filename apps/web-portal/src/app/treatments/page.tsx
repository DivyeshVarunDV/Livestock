...
export default function TreatmentsPage() {
  ...
  const uniqueMedicines = Array.from(new Set(treatments.map(t => t.medicine).filter(Boolean)));
  const uniqueStatuses = Array.from(new Set(treatments.map(t => t.status).filter(Boolean)));

  const filteredTreatments = treatments.filter(t => {
    const matchSearch = 
      t.animalId?.toLowerCase().includes(search.toLowerCase()) || 
      t.medicine?.toLowerCase().includes(search.toLowerCase()) ||
      t.administeredBy?.toLowerCase().includes(search.toLowerCase()) ||
      t.id?.toLowerCase().includes(search.toLowerCase());
    
    const matchStatus = statusFilter ? t.status === statusFilter : true;
    const matchMedicine = medicineFilter ? t.medicine === medicineFilter : true;
      ...
      <div className="bg-white px-8 py-6 border-b border-gray-200">
        ...
        <div className="flex flex-wrap items-center gap-4 mt-6">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg">
            ...
            <span className="text-sm font-bold text-gray-900">{treatments.length}</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-green-50 border border-green-200 rounded-lg">
            <span className="text-sm text-green-700 font-medium">Active:</span>
            <span className="text-sm font-bold text-green-700">{treatments.filter(t => t.status === 'ACTIVE' || t.status === 'Active').length}</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-lg">
            <span className="text-sm text-blue-700 font-medium">Completed:</span>
            <span className="text-sm font-bold text-blue-700">{treatments.filter(t => t.status === 'COMPLETED' || t.status === 'Completed').length}</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-orange-50 border border-orange-200 rounded-lg">
            <span className="text-sm text-orange-700 font-medium">With Withdrawal:</span>
            <span className="text-sm font-bold text-orange-700">{treatments.filter(t => t.withdrawalPeriodDays > 0).length}</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 border border-purple-200 rounded-lg">
            <span className="text-sm text-purple-700 font-medium">AMU Units:</span>
            <span className="text-sm font-bold text-purple-700">
              {treatments.reduce((sum, t) => sum + (t.amuUnits || 0), 0).toFixed(2)}
            </span>
          </div>
        </div>
      </div>
                      ...
                      filteredTreatments.map((t, i) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-gray-50/80 transition-colors">
                            ...
                            <td className="p-4 align-middle text-gray-700">{t.administeredBy || 'N/A'}</td>
                            ...
                            <td className="p-4 align-middle text-gray-700">
                              {t.withdrawalPeriodDays ? `${t.withdrawalPeriodDays} days` : 'None'}
                            </td>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 border-b border-gray-100 pb-4">
              ...
              <div>
                ...
                <span className="text-sm font-bold text-gray-900">{selectedTreatment.animalId || 'N/A'}</span>
              </div>
              <div>
                <span className="text-xs text-gray-500 font-semibold block uppercase">Medicine</span>
                <span className="text-sm font-semibold text-gray-800">{selectedTreatment.medicine || 'N/A'}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="text-sm text-gray-800">{selectedTreatment.administeredBy || 'N/A'}</span>
              </div>
              <div>
                <span className="text-xs text-gray-500 font-semibold block uppercase">Treatment Date</span>
                <span className="text-sm text-gray-800">
                  {selectedTreatment.treatmentDate ? new Date(selectedTreatment.treatmentDate).toLocaleDateString('en-GB') : 'N/A'}
                </span>
              </div>
              <div>
                ...
                <span className="text-sm font-semibold text-orange-700">
                  {(() => {
                    const calc = calculateWithdrawal(selectedTreatment.treatmentDate, selectedTreatment.withdrawalPeriodDays);
